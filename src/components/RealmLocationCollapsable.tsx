...
export default function RealmLocationCollapsable({ locationName, lightItems }: { locationName: string, lightItems: LightItemData[] }) {
    const [isExpanded, setIsExpanded] = useState(false);

    function RealmLocationCollapsableContent() {
        if (lightItems.length === 0) {
            if (locationName !== 'Collected') {
                return (
                    <div className="flex flex-1 flex-col justify-center items-center py-6 select-none cursor-default gap-2">
                        ...
                        <div className="flex flex-col items-center">
                            <p className="dark:text-white/40 text-black/40 transition duration-300 font-semibold text-xl p-0 text-center">You've collected all the light at this location!</p>
                            <p className="dark:text-white/40 text-black/40 text-sm text-center">You can view the light that you've collected under the "Collected" tab at the bottom.</p>
                        </div>
                    </div>
                )
            }
                ...
                return (
                    <div className="flex flex-1 flex-col justify-center items-center py-6 select-none cursor-default">
                        <p className="dark:text-white/40 text-black/40 transition duration-300 font-semibold text-xl p-0 text-center">You haven't collected any light yet!</p>
                        <p className="dark:text-white/40 text-black/40 text-sm text-center">When you collect winged light, it'll show up here.</p>
                    </div>
                )
            }
        }
            ...
            return (
                lightItems.map(item => {
                    return (
                        <LightCard key={item.id} location={locationName} title={item.title} imageUrl={item.imageUrl} youtubeUrl={item.youtubeUrl} onButtonClick={() => { }} />
                    )
                })
            )
        }
    ...
    return (
        ...
        <div className={`${isExpanded ? "bg-neutral-100 dark:bg-neutral-800 pt-4 pb-8" : ""} select-none flex flex-col gap-4 transition-all duration-300 rounded-xl`}>
            ...
            <div className={`${!isExpanded ? "hidden" : ""} ${lightItems.length > 0 ? "pl-10" : ""} flex flex-wrap gap-3 justify-start px-[1.625rem]`}>
